...
$(document).ready(function(e) {
    ...
    $.post('/SmartSocialSecurity/adminInfo',{'action':8},function(data){
		...
		$.each(retdata.account,function(index,entry){
			if(index < 15)
			{
				var newRow = "<tr><td style=\"background-color:#2e75b4;color:white\">"+(index+1)+"</td><td>"+entry.accountnum+"</td><td>"+entry.accountname+"</td><td>"+entry.accountsex+"</td><td>"+entry.accountage+"</td><td>"+entry.accountplace+"</td><td></td><td>"+entry.accountapart+"</td><td>"+entry.accountwork+"</td></tr>";
	...
	for(var i=1;i<16;i++)
		...
		if(page == "首页")
		{
			...
			$.each(retdata.account,function(index,entry){
				if(index < 15)
				{
					var newRow = "<tr><td style=\"background-color:#2e75b4\">"+(i+1)+"</td><td>"+entry.accountnum+"</td><td>"+entry.accountname+"</td><td>"+entry.accountsex+"</td><td>"+entry.accountage+"</td><td>"+entry.accountplace+"</td><td></td><td>"+entry.accountapart+"</td><td>"+entry.accountwork+"</td></tr>";
					$("#list_table tr:last").after(newRow);
				}
			});
		});		
